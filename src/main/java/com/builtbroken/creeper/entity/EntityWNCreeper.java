...
public abstract class EntityWNCreeper extends EntityMob
{
    ...
    public static final String NBT_IGNITE = "ignited";
    public static final String NBT_POWERED = "powered";

    //Data manager keys
    private static final DataParameter<Integer> STATE = EntityDataManager.<Integer>createKey(EntityWNCreeper.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityWNCreeper.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> IGNITED = EntityDataManager.<Boolean>createKey(EntityWNCreeper.class, DataSerializers.BOOLEAN);
    /**
    ...
    /** Used as either a radius or a scale */
    ...
    public void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        this.dataManager.set(POWERED, Boolean.valueOf(compound.getBoolean(NBT_POWERED)));

        if (compound.hasKey(NBT_FUSE, 99))
        {
            this.fuseTime = compound.getShort(NBT_FUSE);
        }

        if (compound.hasKey(NBT_EFFECT_SCALE))
        {
            this.effectScale = compound.getFloat(NBT_EFFECT_SCALE);
        }

        if (compound.getBoolean(NBT_IGNITE))
        {
            this.ignite();
        }
    }
